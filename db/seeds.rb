...
ladies_parent = Category.create(name: 'レディース')
ladies_child_array.each_with_index do |child, i|
  child = ladies_parent.children.create(name: child)
...
# メンズ
mens_child_array = ['トップス','ジャケット/アウター','パンツ','靴','レッグウェア','帽子','バッグ','アクセサリー','小物','時計','浴衣/水着','スーツ/フォーマル','アンダーウェア','その他']
mens_grandchild_array = [
                            ['Tシャツ/カットソー(半袖/袖なし)','Tシャツ/カットソー(七分/長袖)','ポロシャツ','タンクトップ','ニット/セーター','カーディガン','パーカー','トレーナー/スウェット','ベスト','ジャージ','その他'], 
                            ['テーラードジャケット','ノーカラージャケット','Gジャン/デニムジャケット','レザージャケット','ダウンジャケット','ライダースジャケット','ミリタリージャケット','ダウンベスト','ジャンパー/ブルゾン','ポンチョ','ロングコート','トレンチコート','ダッフルコート','ピーコート','チェスターコート','モッズコート','スタジャン','毛皮/ファーコート','スプリングコート','スカジャン','その他'], 
                            ['デニム/ジーンズ','ショートパンツ','カジュアルパンツ','ハーフパンツ','チノパン','ワークパンツ/カーゴパンツ','クロップドパンツ','サロペット/オーバーオール','オールインワン','サルエルパンツ','ガウチョパンツ','その他'], 
                            ['ハイヒール/パンプス','ブーツ','サンダル','スニーカー','ミュール','モカシン','ローファー/革靴','フラットシューズ/バレエシューズ','長靴/レインシューズ','その他'], 
                            ['デニム/ジーンズ','ワークパンツ/カーゴパンツ','スラックス','ショートパンツ','ペインターパンツ','サルエルパンツ', 'オーバーオール','その他'], 
                            ['ニットキャップ/ビーニー','ハット','ハンチング/ベレー帽','キャップ','キャスケット','麦わら帽子','その他'], 
                            ['ハンドバッグ','トートバッグ','エコバッグ','リュック/バックパック','ボストンバッグ','スポーツバッグ','ショルダーバッグ','クラッチバッグ','ポーチ/バニティ','ボディバッグ/ウェストバッグ','マザーズバッグ','メッセンジャーバッグ','ビジネスバッグ','旅行用バッグ/キャリーバッグ','ショップ袋','和装用バッグ','かごバッグ','その他'], 
                            ['ネックレス','ブレスレット','バングル/リストバンド','リング','ピアス(片耳用)','ピアス(両耳用)','イヤリング','アンクレット','ブローチ/コサージュ','チャーム','その他'], 
                            ['長財布','折り財布','コインケース/小銭入れ','名刺入れ/定期入れ','キーケース','キーホルダー','手袋/アームカバー','ハンカチ','ベルト','マフラー/ショール','ストール/スヌード','バンダナ/スカーフ','ネックウォーマー','サスペンダー','サングラス/メガネ','モバイルケース/カバー','手帳','イヤマフラー','傘','レインコート/ポンチョ','ミラー','タバコグッズ','その他'], 
                            ['腕時計(アナログ)','腕時計(デジタル)','ラバーベルト','レザーベルト','金属ベルト','その他'], 
                            ['浴衣','着物','振袖','長襦袢/半襦袢','水着セパレート','水着ワンピース','水着スポーツ用','その他'], 
                            ['スカートスーツ上下','パンツスーツ上下','パーティーバッグ','シューズ','ウェディング','その他'], 
                            ['トップス','アウター','インナー','ワンピース','パンツ','パジャマ','その他'], 
                            ['その他']
                          ]
...
baby_grandchild_array = [
                            ...
                            ['トップス','アウター','パンツ','おくるみ','下着/肌着','パジャマ','ロンバース','その他'], 
                            ['トップス','アウター','パンツ','おくるみ','下着/肌着','パジャマ','ロンバース','その他'],
                            ...
                            ['コート','ジャケット/上着トップス','(Tシャツ/カットソー)','トップス(トレーナー)','トップス(その他)','ボトムス','パジャマ','その他'], 
                            ['スニーカー','サンダル','ブーツ','長靴','その他'], 
                            ['下/スパッツ','帽子','エプロン','サスペンダー','タイツ','ハンカチ','バンダナ','ベルト','マフラー','傘','手袋','スタイ','バッグ','その他'], 
                            ['おむつ用品','おまる/補助便座','トレーニングパンツ','ベビーバス','お風呂用品','その他'], 
                            ['ベビーカー','抱っこひも/スリング','チャイルドシート','その他'], 
                            ['ミルク','ベビーフード','ベビー用食器','その他'], 
                            ['ベッド','布団/毛布','イス','たんす','その他'],
                            ['おふろのおもちゃ','がらがら','オルゴール','ベビージム','手押し車/カタカタ','知育玩具','その他','行事/記念品'],
...
interior_parent = Category.create(name: 'インテリア、住まい、小物')
interior_child_array.each_with_index do |child, i|
  child = interior_parent.children.create(name: child)
...
book_parent = Category.create(name: '本・音楽・ゲーム')
...
toy_grandchild_array = [
                            ...
                            ['アイドル','ミュージシャン','タレント/お笑い','芸人','スポーツ選手','その他'],
                            ...
                            ['トイガン','個人装備','その他'],
                            ['陶芸','ガラス','漆芸','金属工芸','絵画/タペストリ','版画','彫刻/オブジェクト','書写真','その他'],
                            ['画材','額縁','その他'],
...
toy_parent = Category.create(name: 'おもちゃ・ホビー・グッズ')
toy_child_array.each_with_index do |child, i|
  child = toy_parent.children.create(name: child)
...
cosmetics_parent = Category.create(name: 'コスメ・香水・美容')
cosmetics_child_array.each_with_index do |child, i|
  child = cosmetics_parent.children.create(name: child)
...
# 家電・スマホ・カメラ
homeappliances_child_array = ['スマートフォン/携帯電話','スマホアクセサリー','PC/タブレット','カメラ','テレビ/映像機器','オーディオ機器','美容/健康','冷暖房/空調','生活家電']
...
sports_grandchild_array = [
                            ...
                            ['板ブーツ(男性用)','ブーツ(女性用)','ブーツ(子ども用)','ビンディングウエア(男性用)','ウエア(女性用)','ウエア(子ども用)','ストック','その他'],
...
sports_parent = Category.create(name: 'スポーツ・レジャー')
sports_child_array.each_with_index do |child, i|
  child = sports_parent.children.create(name: child)
...
handmade_grandchild_array = [
                            ...
                            ['バッグ(女性用)','バッグ(男性用)','財布(女性用)','財布(男性用)','ファッション','雑貨','その他'],
                            ...
                            ['Ingressク','リエイターズ','宇宙兄弟'],
                          ]
handmade_parent = Category.create(name: 'ハンドメイド')
handmade_child_array.each_with_index do |child, i|
  child = handmade_parent.children.create(name: child)
...
# チケット
ticket_child_array = ['音楽','スポーツ','演劇/芸能','イベント','映画','施設利用券','優待券/割引券']
ticket_grandchild_array = [
                            ['男性アイドル','女性アイドル','韓流国内アーティスト','海外アーティスト','音楽フェス','声優/アニメ','その他'],
                            ['サッカー','野球','テニス','格闘技/プロレス','相撲/武道','ゴルフ','バレーボール','バスケットボール','モータースポーツ','ウィンタースポーツ','その他'],
                            ['ミュージカル','演劇','伝統','芸能','落語','お笑い','オペラ','サーカス','バレエ','その他'],
                            ['声優/アニメ','キッズ/ファミリー','トークショー/講演会','その他'], 
                            ['邦画','洋画','その他'], 
                            ['遊園地/テーマパーク','美術館/博物館','スキー場','ゴルフ場','フィットネスクラブ','プール','ボウリング場','水族館','動物園','その他'], 
                            ['ショッピング','レストラン/食事券','フード/ドリンク','券宿泊券','その他']
                          ]
ticket_parent = Category.create(name: 'チケット')
ticket_child_array.each_with_index do |child, i|
  child = ticket_parent.children.create(name: child)
...
# 自動車・オートバイ
car_child_array = ['自動車本体','自動車タイヤ/ホイール','自動車パーツ','自動車パーツ','オートバイ車体','オートバイパーツ','オートバイアクセサリー']
car_grandchild_array = [
                            ['国内自動車本体','外国自動車本体'],
                            ['タイヤ/ホイールセット','タイヤ','ホイール','その他'],
                            ['サスペンションブレーキ外装','エアロパーツライト内装品','シートステアリングマフラー・排気系エンジン','過給器','冷却装置ク','ラッチ,ミッション,駆動系','電装品','補強パーツ','汎用パーツ','外国自動車用パーツ','その他'],
                            ['車内アクセサリー','カーナビ','カーオーディオ','車外アクセサリー','メンテナンス用品','チャイルドシート','ドライブレコーダー','レーダー探知機','カタログ/マニュアル','セキュリティ','ETC','その他'], 
                            ['オートバイ車体'], 
                            ['タイヤマフラーエンジン','冷却装置','カウル','フェンダー','外装','サスペンション','ホイール','シート','ブレーキ','タンク','ライト、ウィンカー','チェーン、スプロケット、駆動系','メーター','電装系','ミラー','外国オートバイ用パーツ','その他'], 
                            ['ヘルメット/シールド','バイクウエア/装備','アクセサリー','メンテナンス','カタログ/マニュアル','その他']
                          ]
car_parent = Category.create(name: '自動車・オートバイ')
car_child_array.each_with_index do |child, i|
  child = car_parent.children.create(name: child)
...
# その他
other_child_array = ['まとめ売り','ペット用品','食品','飲料/酒','日用品/生活雑貨/旅行','アンティーク/コレクション','文房具/事務用品','事務/店舗用品']
other_grandchild_array = [
                            ...
                            ['コーヒー','ソフトドリンク','ミネラルウォーター','茶ウイスキー','ワイン','ブランデー','焼酎','日本酒','ビール、発泡酒','その他'], 
...
other_parent = Category.create(name: 'その他')
other_child_array.each_with_index do |child, i|
  child = other_parent.children.create(name: child)
